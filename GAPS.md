...
| 2026-08-25 | a terminal query reply (`[1;1R`, cursor position) is typed into the input on start — glamour `auto` style/termenv queries the terminal after Bubble Tea owns stdin | mote tui | now | worse than a leak: under a pty that never answers, the demo writes the two queries and draws nothing (blocked). Milestone 3, item 1 |
