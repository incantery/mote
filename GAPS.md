...
| 2026-08-25 | a terminal query reply (`[1;1R`, cursor position) is typed into the input on start — glamour `auto` style/termenv queries the terminal after Bubble Tea owns stdin | mote tui | now | fixed: nothing mote draws asks the terminal anything. `resolveStyle` settles the style in `New` from GLAMOUR_STYLE/profile/COLORFGBG, and answers lipgloss too, so `AdaptiveColor` stops asking (065c8b59) |
